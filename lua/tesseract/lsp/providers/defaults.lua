...
local M = {}
local augroup_name = 'TesseractNvimLspFormat'
...
function M.on_attach(client, bufnr)
  ...
  if client.supports_method('textDocument/formatting') then
    ...
    vim.cmd(string.format("command! LspFormat lua require('tesseract.utils.lsp').format(%s)", bufnr))
    ...
    if user_config.lsp.format_on_save then
      ...
      vim.api.nvim_create_autocmd(string.format('BufWritePre %s', format_filetypes), {
        callback = function()
          require('tesseract.utils.lsp').format(bufnr)
        end,
        group = group,
      })
    end
  ...
  require('tesseract.lsp.mappings').init(client, bufnr)
