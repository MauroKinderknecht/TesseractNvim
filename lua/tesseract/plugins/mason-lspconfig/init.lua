return {
  ...
  config = function()
    ...
    local default_config = require('tesseract.lsp.providers.defaults')
    ...
    local start_server = function(server)
      ...
      -- set up default tesseract options
      if server == 'tsserver' then
        opts = u.merge(opts, require('tesseract.lsp.providers.tsserver'))
      elseif server == 'jsonls' then
        opts = u.merge(opts, require('tesseract.lsp.providers.jsonls'))
      elseif server == 'pyright' then
        opts = u.merge(opts, require('tesseract.lsp.providers.pyright'))
      elseif server == 'sumneko_lua' then
        opts = u.merge(opts, require('tesseract.lsp.providers.sumneko_lua'))
      end
