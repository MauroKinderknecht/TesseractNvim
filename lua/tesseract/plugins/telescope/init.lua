...
return {
  ...
  config = function()
    require('tesseract.plugins.telescope.config')
  end,
  init = function()
    -- normal mappings
    require('tesseract.plugins.telescope.mappings').init()

    user_config.lsp.add_on_attach_mapping(function(client, bufnr)
      local map = require('tesserract.utils').map
  ...
  keys = {
    {
      '<leader>ff',
      '<cmd>lua require("tesseract.plugins.telescope.mappings").project_files()<cr>',
